...
begin
  ...
  if lHash[iPos] <> nil then begin                                                                             //Verifica se existe o elemento
    ...
    if (subHash^.sDado = sNum) and (subHash^.pProx = nil) and (lHash[iPos] = subHash) then begin               //Se existe apenas um elemento
      lHash[iPos]:= nil;
      Dispose(subHash);
    end else if (subHash^.pProx <> nil) and (lHash[iPos] = subHash) then begin                                 //Se é o primeiro elemento
      lHash[iPos]:= subHash^.pProx;
      Dispose(subHashAnt);
    end else if (subHash <> lHash[iPos]) and (subHash^.sDado = sNum) and (subHash^.pProx = nil) then begin     //Se é o ultimo elemento
